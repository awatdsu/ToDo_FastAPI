...
from app.tasks.schemas import TTaskResponse
from app.tasks.dao import TaskDao
from app.users.dependencies import get_current_user
from app.users.dao import UserDAO
from app.models import User
from app.tasks.rb import RBTask
...
async def add_new_task(username: str, current_user: User = Depends(get_current_user), task_data: RBTask = Depends()):
    res = await UserDAO.find_one_or_none_by_username(username)
    if res is None or current_user.id != res.id:
        raise HTTPException(status_code=403, detail="Forbidden")
    
    await TaskDao.add_new_task(task_owner=current_user, **task_data.to_dict())
    return {"detail":"Заметка успешно создана!"}

@router.delete("/{task_id}/deleteToDo", summary="Удаление заметки", responses={
    200: {"model": SuccesfulResponse},
    403: {"model" : ErrorResponse}
})
